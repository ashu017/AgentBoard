...
- When you FINISH, call submit_result with your output, and set status to done (or failed if it didn't work). If your work opened a pull request, pass its URL as pr_url so it shows on the review card. Before you raise that PR, ALWAYS sync with the default branch first — fetch the latest main and merge (or rebase) it into your branch, resolve any conflicts, and re-run the build/tests — so the PR is conflict-free and green. A PR that conflicts with main isn't done.
