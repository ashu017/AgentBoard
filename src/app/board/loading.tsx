...
// Mirror ProjectView's 4 columns (Todo / Running / Needs Review / Done) with
// their status accent colors, and a per-column count of placeholder cards of
// varying heights so it reads like a populated board mid-load.
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden" aria-busy="true" aria-live="polite">
      ...
      {/* Header strip — mirrors the real Header (bg-paper-2, bottom rule):
          wordmark block, workspace + idea-switcher blocks, and the review/New/
          sign-out cluster on the right. */}
      <header
      ...
      {/* Board body — mirrors ProjectView: a project-header row (title +
          stat blocks + completion bar) above the 4 kanban columns. */}
        ...
        <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          {COLUMNS.map(({ label, accent, cards }) => (
            <div key={label} className="clip-corner border border-line bg-paper">
              ...
              {/* Placeholder cards of varying heights. */}
              <div className="space-y-2 p-2">
                {cards.map((h, i) => (
                  <div
                    key={i}
                    className="clip-corner border border-line bg-paper p-2.5"
                    style={{ minHeight: h }}
                  >
